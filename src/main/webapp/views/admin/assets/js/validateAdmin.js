...
function validateLogin() {
	...
	if (username.length < 5) {
		showSwalAlert('error', 'Tên đăng nhập phải có ít nhất 6 ký tự !');
		return false;
	}
	if (password == "") {
...
function validateNewCategory() {
	...
	if (name == "" ) {
		showSwalAlert('error', 'Thể loại không thể để trống!');
		return false;
	}
...
function editCategory() {
	Swal.fire({
		title: 'Cảnh Báo !',
		text: "Bạn có chắc chắn chỉnh sửa category không ?",
		icon: 'warning',
		showCancelButton: true,
		confirmButtonColor: '#3085d6',
		cancelButtonColor: '#d33',
