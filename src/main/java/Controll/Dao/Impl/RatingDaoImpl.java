...
public class RatingDaoImpl extends AbstactDao<Rating> implements RatingDao {
	...
	public List<Rating> findByEmail(String email) {
		String sql = "SELECT o FROM Rating o WHERE o.user.email = ?0 AND o.movie.isActive = 1 ORDER BY o.ratingDate DESC";
		return super.findMany(Rating.class, sql, email);
	}

	@Override
	public Rating findByUserIdAndMovieId(Integer userId, Integer movieId) {
		String sql = "SELECT o FROM Rating o WHERE o.user.id = ?0 AND o.movie.id = ?1 AND o.movie.isActive = 1";
		return super.findOne(Rating.class, sql, userId, movieId);
	}
