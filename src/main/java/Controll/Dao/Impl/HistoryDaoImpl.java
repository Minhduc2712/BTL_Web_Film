...
public class HistoryDaoImpl extends AbstactDao<History> implements HistoryDao {
	...
	public List<History> findByUser(String email) {
		String sql = "SELECT o FROM History o WHERE o.user.email = ?0 AND o.movie.isActive = 1"
	...
	public List<History> findByUserAndIsLiked(String email) {
		String sql = "SELECT o FROM History o WHERE o.user.email = ?0 AND o.isLiked = 1" + " AND o.movie.isActive = 1"
				+ " ORDER BY o.viewedDate DESC";
		return super.findMany(History.class, sql, email);
	}

	@Override
	public History findByUserIdAndMovieId(Integer userId, Integer videoId) {
		String sql = "SELECT o FROM History o WHERE o.user.id = ?0 AND o.movie.id = ?1 AND o.movie.isActive = 1";
